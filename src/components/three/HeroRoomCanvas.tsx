import { Edges } from "@react-three/drei";
import { Canvas, useFrame } from "@react-three/fiber";
import { MathUtils, type Group } from "three";
import { useRef } from "react";

import type { DeviceTier } from "../../hooks/useDeviceTier";

type HeroRoomCanvasProps = {
  active: boolean;
  onError: () => void;
  onReady: () => void;
  tier: Exclude<DeviceTier, "low">;
};

type ModularRoomProps = {
  active: boolean;
  tier: Exclude<DeviceTier, "low">;
};

function RoomPart({
  args,
  color,
  position,
}: {
  args: [number, number, number];
  color: string;
  position: [number, number, number];
}) {
  return (
    <mesh castShadow receiveShadow position={position}>
      <boxGeometry args={args} />
      <meshStandardMaterial color={color} roughness={0.72} metalness={0.08} />
      <Edges color="#858780" opacity={0.38} transparent />
    </mesh>
  );
}

function ModularRoom({ active, tier }: ModularRoomProps) {
  const roomRef = useRef<Group>(null);
  const startTime = useRef<number | null>(null);

  useFrame(({ clock, invalidate }) => {
    if (!active || !roomRef.current) {
      return;
    }

    startTime.current ??= clock.elapsedTime;
    const progress = Math.min(
      (clock.elapsedTime - startTime.current) / 1.15,
      1,
    );
    const easedProgress = 1 - Math.pow(1 - progress, 3);

    roomRef.current.position.y = MathUtils.lerp(-0.35, 0, easedProgress);
    roomRef.current.rotation.y = MathUtils.lerp(-0.16, -0.08, easedProgress);

    if (progress < 1) {
      invalidate();
    }
  });

  return (
    <group ref={roomRef} rotation={[-0.04, -0.08, 0]}>
      <gridHelper
        args={[8, 16, "#595b56", "#252623"]}
        position={[0, -1.08, 0]}
      />
      <RoomPart args={[5.2, 0.16, 4.1]} color="#252623" position={[0, -1, 0]} />
      <RoomPart
        args={[5.2, 2.7, 0.16]}
        color="#30312d"
        position={[0, 0.28, -1.98]}
      />
      <RoomPart
        args={[0.16, 2.7, 4.1]}
        color="#191a18"
        position={[-2.52, 0.28, 0]}
      />
      <RoomPart
        args={[0.12, 1.9, 0.78]}
        color="#b95d39"
        position={[-2.36, 0.32, -0.65]}
      />
      <RoomPart
        args={[2.15, 0.68, 0.88]}
        color="#858780"
        position={[0.45, -0.6, 0.2]}
      />
      <RoomPart
        args={[0.72, 1.25, 0.72]}
        color="#f5f3ec"
        position={[1.55, -0.3, -1.48]}
      />

      <hemisphereLight intensity={tier === "high" ? 1.4 : 1.1} />
      <directionalLight
        castShadow={tier === "high"}
        intensity={2.1}
        position={[4, 6, 4]}
        shadow-mapSize-height={tier === "high" ? 1024 : 512}
        shadow-mapSize-width={tier === "high" ? 1024 : 512}
      />
      <pointLight color="#e8a182" intensity={8} position={[-1.8, 1, 0]} />
    </group>
  );
}

export default function HeroRoomCanvas({
  active,
  onError,
  onReady,
  tier,
}: HeroRoomCanvasProps) {
  return (
    <div className="hero-scene__canvas" aria-hidden="true">
      <Canvas
        camera={{ fov: 38, position: [5.8, 4.1, 7.2] }}
        dpr={tier === "high" ? [1, 1.5] : [1, 1.25]}
        frameloop={active ? "demand" : "never"}
        gl={{
          antialias: tier === "high",
          powerPreference: "high-performance",
        }}
        onCreated={({ gl }) => {
          gl.setClearColor("#111210", 0);
          gl.domElement.addEventListener("webglcontextlost", onError, {
            once: true,
          });
          onReady();
        }}
        shadows={tier === "high"}
      >
        <ModularRoom active={active} tier={tier} />
      </Canvas>
    </div>
  );
}
